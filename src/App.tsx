...
import { streamAIMessage } from "./utils/streamAI";
...
function App() {
  ...
  // Helper to get current chat session
  ...
  const sendMessage = async () => {
    if (!address || !message.trim() || isLoading || !currentSession) return;
    ...
    let aiText = "";
    try {
      const signature = await signMessageAsync({ message: "Sign in to Monad Swap" });
      await streamAIMessage(
        "http://localhost:3000/swap-stream",
        { user: address, message, signature },
        (chunk) => {
          aiText += chunk;
          setSessions((prev) =>
            prev.map((s) =>
              s.id === currentSession.id
                ? {
                    ...s,
                    messages: [
                      ...s.messages.filter((m, i) =>
                        i !== s.messages.length - 1 || m.role !== "ai"
                      ),
                      ...(aiText
        ...
        prev.map((s) =>
          s.id === currentSession.id
            ? {
                ...
                messages: [
                  ...s.messages,
                  { role: "ai" as const, text: "Oops! Something went wrong. Please try again." },
                ],
              }
            : s
        )
  ...
  if (showLanding) {
    return <LandingPage onGetStarted={() => {
      ...
      if (!sessions.length) handleNewChat();
    }} />;
  }

  return (
    <div className="min-h-screen bg-black bg-black-cyan-gradient flex flex-row font-sans">
      {/* Sidebar */}
      <Sidebar
        chats={sessions.map(({ id, title }) => ({ id, title }))}
        currentChatId={currentChatId}
        onSelectChat={handleSelectChat}
        onNewChat={handleNewChat}
      />
      {/* Main Chat Area */}
      <div className="flex flex-col flex-1 items-center justify-between p-4 md:p-8">
        {/* Header */}
        <motion.header
          ...
          {isConnected && currentSession ? (
            ...
            >
              ...
              <div className="flex-grow p-4 md:p-6 overflow-y-auto space-y-4 custom-scrollbar">
                ...
                {currentSession.messages.map((c, i) => (
                  <motion.div
                    key={i}
                    initial="hidden"
                    animate="visible"
            ...
            >
              <p>
                Connect your wallet to start chatting with Monad AI and make swaps!
              </p>
            </motion.div>
          )}
        </AnimatePresence>
